...
默认 8 MB DOS 内存、2750 固定 cycles、静音。设置可选 16 MB、4720/7800 cycles、铺满宽度和声音。提高 cycles 会增加主机负担，不保证更快。声音接口保留，但遵照用户工作时间要求尚未测试发声和音画同步。
...
暂停菜单可继续、切换输入方式／画幅和返回游戏库。没有屏幕虚拟键盘。默认保持 4:3；DOS 命令行自动铺满宽度以显示 80 列。显示最多约 30 次/秒，模拟核心按其刷新率推进；这不等于游戏性能保证。
