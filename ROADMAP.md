...
2026-09-22。目标：MIPS32r2 Linux、128 MB RAM、800×340、触屏与实体键盘。本文件是开发规划；以下四个新应用尚未实现或安装。
...
内存数字是设计预算，不是实测结果。所有应用沿用 apps 独立数据目录、原子写存档、启动停止原桌面、短按电源回菜单；不添加屏幕键盘。PDF 固定版式、扫描 OCR、复杂 EPUB CSS、PS 模拟器不作为首批目标。
