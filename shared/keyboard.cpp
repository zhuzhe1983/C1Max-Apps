...
void close(){for(auto&fd:fds){if(fd>=0)::close(fd);fd=-1;}queue.clear();mapper.reset();dropped[0]=dropped[1]=false;}
void open(){close();fds[0]=::open("/dev/input/event0",O_RDONLY|O_NONBLOCK|O_CLOEXEC);fds[1]=::open("/dev/input/event1",O_RDONLY|O_NONBLOCK|O_CLOEXEC);input_event e;for(auto fd:fds)while(fd>=0&&read(fd,&e,sizeof(e))==sizeof(e)){} }
