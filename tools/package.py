...
ids=['launcher','piano','nes','streamplayer','calendar','calculator','terminal','gomoku','pcsx4all','processing','dosbox','airtune','crosspoint','camera','mail','bilibili']
...
for name in ids:
    ...
    apps.append({'id':name,'version':'0.3.1' if name=='crosspoint' else '0.3.0' if name=='calendar' else '0.1.2' if name=='dosbox' else '0.1.1' if name in ['pcsx4all','bilibili'] else '0.1.0' if name in ['terminal','gomoku','processing'] else '0.2.0','revision':digest.hexdigest()})
